...
if ($action == 'activate') {
    if (ALO_em_edit_subscriber_state_by_email($email, "1", $unikey)) {
        echo "<p>Your subscription was successfully activated. You'll receive the next newsletter. Thank you.</p>";
    } else {
        echo "<p>Error during activation. Please check the activation link.</p>";
    }
}
...
if ($action == 'unsubscribe') {
    if (ALO_em_delete_subscriber_by_email($email, $unikey)) {
        echo "<p>Your subscription was successfully deleted. Bye bye.</p>";;
    } else {
        echo "<p>Error during unsubscription. Try again.</p>";
